...
class Stream {
    ...
    public get(pos?:number):string|number {
        if (pos === undefined) {
            pos = this.pos++;
        }
    ...
    public hexByte(b:number|string) {
    ...
    public isASCII(start:number, end:number):boolean {
        for (let i = start; i < end; ++i) {
            const c = this.get(i);
            if (c < 32 || c > 176) {
                return false;
    ...
    public parseTime(start:number, end:number, shortYear) {
    ...
    private enc:string|string[];
    private pos:number;

    private hexDigits = "0123456789ABCDEF";
}
...
export class ASN1 {
    ...
    public toHexString(root?:any) {
        return this.stream.hexDump(this.posStart(), this.posEnd(), true);
    }


    public static decodeLength(stream):number {
        var buf = stream.get(),
            len = buf & 0x7F;
        if (len == buf)
            return len;
...
class ASN1Tag {
    ...
    private tagClass:number;
    private tagConstructed:boolean;
    private tagNumber:number|Int10;
}
